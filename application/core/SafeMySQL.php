...
class SafeMySQL extends \PDO
{
	...
	private $defaults = array(
		...
		'exception' => '\application\core\DbException', //Exception class name
	);
	...
	function __construct($opt = array())
	{
		...
		parent::__construct("mysql:host=".$opt['host'].";dbname=".$opt['db'], $opt['user'], $opt['pass']);
		// "mysql:host=localhost;dbname=c14_hospitab-new", "c14_hospitab-new", "YAPG57iU"
		...
		if ( !$this->conn )
		{
			$this->error(array('code' => mysqli_connect_errno(), 'message' => mysqli_connect_error()));
		}

		mysqli_set_charset($this->conn, $opt['charset']) or $this->error(array('code' => mysqli_errno($this -> conn), 'message' => mysqli_error($this->conn)));
	...
	private function rawQuery($query)
	{
		...
		if (!$res)
		{
			$error = array('code' => mysqli_errno($this->conn), 'message' => mysqli_error($this->conn), 'query' => $query);
	...
	private function prepareQuery($args)
	{
		...
		if ( $pnum != $anum )
		{
			$this->error(array('code' => 7100, 'message' => "Number of args ($anum) doesn't match number of placeholders ($pnum) in [$raw]"));
		}
	...
	private function escapeInt($value)
	{
		...
		if(!is_numeric($value))
		{
			$this->error(array('code' => 7101, 'message' => "Integer (?i) placeholder expects numeric value, ".gettype($value)." given"));
			return FALSE;
		}
		if (is_float($value))
		{
	...
	private function escapeIdent($value)
	{
		if ($value)
		{
			...
			$this->error(array('code' => 7102, 'message' => "Empty value for identifier (?n) placeholder"));
		}
	}
	...
	private function createIN($data)
	{
		if (!is_array($data))
		{
			$this->error(array('code' => 7103, 'message' => "Value for IN (?a) placeholder should be array"));
			return;
		}
		if (!$data)
		{
	...
	private function createSET($data)
	{
		if (!is_array($data))
		{
			$this->error(array('code' => 7104, 'message' => "SET (?u) placeholder expects array, ".gettype($data)." given"));
			return;
		}
		if (!$data)
		{
			$this->error(array('code' => 7105, 'message' => "Empty array for SET (?u) placeholder"));
			return;
		}
		$query = $comma = '';
	...
	{
		$error  = __CLASS__.": ".print_r($err, TRUE);

		if ( $this->emode == 'error' )
		{
			$error .= ". Error initiated in ".$this->caller().", thrown";
			trigger_error($error,E_USER_ERROR);
		} else {
			$query = array_key_exists('query', $err) ? $err['query'] : '';
			$trace = $this -> caller();
			throw new $this->exname($err['code'], $err['message'], $trace['file'], $trace['line'], $query);
		}
	}
		...
		foreach ($trace as $t)
		{
			if ( isset($t['class']) && $t['class'] == __CLASS__ )
			{
				if($this -> emode == 'error'){
					...
					$caller = array('file' => $t['file'], 'line' => $t['line']);
				}
			} else {
				break;
			}
