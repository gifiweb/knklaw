...
class Auth
{
	...
	*/
	...
	{
		$query = $this->dbh->prepare("SELECT username, password, email, type, salt, isactive, firstname, lastname, room, parent_id FROM {$this->config->table_users} INNER JOIN user_profiles WHERE id = uid AND id = ?");
		$query->execute(array($uid));
	...
	*/
	...
	public function validateUsername($username) {
		...
		$bannedUsernames = file(__DIR__ . "/../lib/banned-usernames.txt", FILE_IGNORE_NEW_LINES);
	...
	*/
	...
	private function validateEmail($email) {
		...
		$bannedEmails = file(__DIR__ . "/../lib/banned-emails.txt", FILE_IGNORE_NEW_LINES);
